...
def insert_book(title: str, author: str, isbn: str, total_copies: int, available_copies: int) -> bool:
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"[DEBUG] Successfully inserted book: {title}")
        return True
    except Exception as e:
        conn.close()
        print(f"[ERROR] Failed to insert book: {title}, Error: {e}")
        return False
...
def get_borrowing_history(patron_id: str) -> List[Dict]:
    ...
    for record in records:
        ...
        is_overdue = False
        if not return_date and datetime.now() > due_date:
            is_overdue = True
        elif return_date and return_date > due_date:
            is_overdue = True
...
def reset_database():
    ...
    cur.execute("DELETE FROM sqlite_sequence WHERE name='borrow_records'")
